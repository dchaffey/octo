...
std::string Chicken::get_name() const {
  return name;
}

int Chicken::get_age() const {
  return age;
}

int Chicken::get_speed() const {
  return speed_kmh;
}

int Chicken::get_energy() const {
  return energy_level;
}

int Chicken::get_eggs_laid() const {
  return eggs_laid;
}
...
void Chicken::lay_egg() {
  assert(energy_level >= 30 && "chicken must have at least 30 energy to lay egg");

  energy_level -= 30;
  eggs_laid++;
}

void Chicken::age_one_week() {
  age++;
}
