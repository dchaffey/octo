...
pub fn tuiMain(init: std.process.Init, tui_inbox: *std.Io.Queue(UiMsg)) !void {
    const io = init.io;
    _ = tui_inbox;

    var vaxis_arena = std.heap.ArenaAllocator.init(std.heap.page_allocator);
    defer vaxis_arena.deinit();
    const vaxis_alloc = vaxis_arena.allocator();

    var buffer: [1024]u8 = undefined;
    var app: vxfw.App = try .init(io, vaxis_alloc, init.environ_map, &buffer);
    defer app.deinit();

    // We heap allocate our model because we will require a stable pointer to it in our Button
    // widget
    const model = try vaxis_alloc.create(Model);
    defer vaxis_alloc.destroy(model);
